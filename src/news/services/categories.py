...
class CategoryService:
    ...
    async def update_category(
        ...
        """
        if partial:
            category = await DBManager.partial_update_object(**category, db=db, model=Category, field="id",
                                                             value=category_id, commit=True)
        else:
            category = await DBManager.update_object(**category, db=db, model=Category, field="id", value=category_id,
                                                     commit=True)
